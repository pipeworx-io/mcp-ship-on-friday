...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'ship-on-friday');
}

// No credential on purpose. The endpoint answers a bare request (verified
// 2026-09-21: HTTP 200 with a real payload and no API-key header). A shared
// 48-hex key used to be hardcoded here across 31 sibling packs (fleet #2289).
// If StupidAPIs ever starts requiring one, wire a PLATFORM_ key through the
// manifest — never a literal.
const HEADERS = { 'User-Agent': 'pipeworx-ship-on-friday/1.0 (+https://pipeworx.io)' };
...
async function callApi(url: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  const res = await pwFetch(fullUrl, {
    headers: HEADERS,
  });
