...
`tools/list` at `https://gateway.pipeworx.io/ship-on-friday/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/ship_on_friday_check \
  -H 'Content-Type: application/json' \
  -d '{"deploy_type":"hotfix","team_size":3}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/ship_on_friday_check`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "ship-on-friday": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-ship-on-friday"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-ship-on-friday
```
